...
def train(method):
    ...
    for domain in config["source_domains"]:
        ...
        source_loaders.append(
            DataLoader(train_sub, batch_size=config["batch_size_per_source"],
    ...
    if method == "dann":
        ...
        domain_disc = DomainDiscriminator(in_features=512 * config["num_classes"]).to(device)
        params += list(domain_disc.parameters())
    ...
    history = {
        "epoch": [],
        "cls_loss": [],
        "align_loss": [],   # MMD or domain loss (0 for source_only)
        "total_loss": [],
        "mean_source_f1": [],
        "mean_source_acc": [],
    }
    ...
    for epoch in range(config["max_epochs"]):
        ...
        for batch_idx in range(total_batches):
            ...
            epoch_align_loss += align_loss_val
        ...
        with torch.no_grad():
            for domain in config["source_domains"]:
                ...
                history[f"{domain}_f1"].append(macro_f1)
        ...
        history["cls_loss"].append(epoch_cls_loss / num_batches)
        history["align_loss"].append(epoch_align_loss / num_batches)
        ...
        print(
            f"Epoch {epoch+1:3d} | "
            f"cls_loss={epoch_cls_loss/num_batches:.4f} | "
            f"align_loss={epoch_align_loss/num_batches:.4f} | "
            f"mean_src_F1={mean_f1:.4f} | "
            f"mean_src_Acc={mean_acc:.4f}"
        )
